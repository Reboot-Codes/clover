...
impl ManifestCompilationFrom<String> for RequiredString {
  async fn compile(spec: String, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized {
    let mut err = None;

    let res = match resolve_entry_value(spec.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(resolution) => {
        match resolution {
          Resolution::ImportedMultiple(_) => {
            err = Some(SimpleError::new("Glob import not supported at this level!"));
...
impl ManifestCompilationFrom<OptionalStringListManifestSpecEntry> for OptionalStrStrHashMap {
  async fn compile(spec: OptionalStringListManifestSpecEntry, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized {
    let mut err = None;
...
impl<T: Clone + std::fmt::Debug, K> ManifestCompilationFrom<OptionalListManifestSpecEntry<T>> for OptionalStrTHashMap<K> where
  K: ManifestCompilationFrom<T> + std::fmt::Debug, T: for<'a> Deserialize<'a>
{
  async fn compile(spec: OptionalListManifestSpecEntry<T>, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized {
    let mut err = None;
    ...
    match spec {
        OptionalListManifestSpecEntry::Some(hash_map) => {
          debug!("OptionalListManifestSpecEntry::Some: {:#?}", hash_map.clone());
        ...
        OptionalListManifestSpecEntry::ImportString(raw_str) => {
          debug!("OptionalListManifestSpecEntry::ImportString: {}", raw_str.clone());

          match resolve_entry_value(raw_str.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
            Ok(resolution) => {
              match resolution {
                ...
                Resolution::NoImport(_) => {
                  err = Some(SimpleError::new("A string is not a valid value for this field unless it's an import."));
                },
              }
            },
            Err(e) => {
              debug!("OptionalListManifestSpecEntry::ImportString => resolve_list_entry(), failed: {}", e);
              err = Some(e);
            }
          }
        },
...
impl<T: Clone + for<'a> Deserialize<'a>, K: ManifestCompilationFrom<T> + for<'a> Deserialize<'a>> ManifestCompilationFrom<RequiredListManifestSpecEntry<T>> for RequiredStrTHashMap<K> {
  async fn compile(spec: RequiredListManifestSpecEntry<T>, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized {
    let mut err = None;

    let res = match spec.clone() {
      RequiredListManifestSpecEntry::ImportString(spec_str) => {
        match resolve_entry_value(spec_str.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
          Ok(resolution) => {
            match resolution {
              Resolution::ImportedMultiple(_) => {
                err = Some(SimpleError::new("Glob import not supported at this level!"));
      ...
      RequiredListManifestSpecEntry::Some(spec_list) => {
        ...
        for (entry_id, raw_entry) in spec_list {
          match raw_entry {
            ...
            RequiredSingleManifestEntry::ImportString(raw_entry_str) => {
              match resolve_entry_value(raw_entry_str, resolution_ctx.clone(), repo_dir_path.clone()).await {
                Ok(resolution) => {
                  match resolution {
                    Resolution::ImportedSingle((here, val_str)) => {
                      match serde_jsonc::from_str(&val_str) {
...
impl ManifestCompilationFrom<OptionalSingleManifestSpecEntry<bool>> for OptionalBoolean {
  async fn compile(spec: OptionalSingleManifestSpecEntry<bool>, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized, Option<bool>: for<'a> Deserialize<'a> {
    let mut err = None;
...
impl Manifest {
  pub async fn compile(spec: ManifestSpec, spec_path: OsPath, repo_dir_path: OsPath) -> Result<Manifest, SimpleError> {
    let mut err = None;
...
impl ManifestCompilationFrom<RawApplicationSpec> for ApplicationSpec {
  async fn compile(spec: RawApplicationSpec, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized {
    ...
    let intents = match OptionalStrStrHashMap::compile(spec.intents.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(val) => {
    ...
    let containers  = match OptionalStrTHashMap::compile(spec.containers.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(val) => {
...
impl ManifestCompilationFrom<RawContainerSpec> for ContainerSpec {
  async fn compile(spec: RawContainerSpec, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized, RawContainerSpec: for<'a> Deserialize<'a> {
    let mut err = None;

    debug!("Resolving container.interface");
    let interface = match OptionalBoolean::compile(spec.interface.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(val) => {
    ...
    let build = match Optional::compile(spec.build.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(val) => {
...
impl ManifestCompilationFrom<RawBuildConfig> for BuildConfig {
  async fn compile(spec: RawBuildConfig, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized, RawBuildConfig: for<'a> Deserialize<'a> {
    let mut err = None;

    debug!("Resolving build-config.url");
    let url_field = match RequiredString::compile(spec.url.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(val) => {
    ...
    let creds = match Optional::compile(spec.creds.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(val) => {
...
impl ManifestCompilationFrom<RawRepoCreds> for RepoCreds {
  async fn compile(spec: RawRepoCreds, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized, RawRepoCreds: for<'a> Deserialize<'a> {
    let mut err = None;

    debug!("Resolving repo-creds.username");
    let username = match OptionalString::compile(spec.username.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(val) => {
    ...
    let key = match RequiredString::compile(spec.key.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(val) => {
...
impl ManifestCompilationFrom<RawModuleSpec> for ModuleSpec {
  async fn compile(spec: RawModuleSpec, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized {
    let mut err = None;
...
impl ManifestCompilationFrom<RawExpressionPackSpec> for ExpressionPackSpec {
  async fn compile(spec: RawExpressionPackSpec, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized {
    let mut err = None;
    ...
    let expressions = match OptionalStrTHashMap::compile(spec.expressions.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(val) => {
...
impl ManifestCompilationFrom<RawExpressionSpec> for ExpressionSpec {
  async fn compile(spec: RawExpressionSpec, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized, RawExpressionSpec: for<'a> Deserialize<'a> {
    let mut err = None;
...
impl ManifestCompilationFrom<RawStaticExpressionSpec> for StaticExpressionSpec {
  async fn compile(spec: RawStaticExpressionSpec, resolution_ctx: ResolutionCtx, repo_dir_path: OsPath) -> Result<Self, SimpleError> where Self: Sized, RawStaticExpressionSpec: for<'a> Deserialize<'a> {
    let mut err = None;

    debug!("Resolving static-expression-pack.static-url");
    let static_url = match RequiredString::compile(spec.static_url.clone(), resolution_ctx.clone(), repo_dir_path.clone()).await {
      Ok(val) => {
